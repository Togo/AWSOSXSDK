...
+ (AmazonServiceRequest *)createRequest:(SNSGetTopicAttributesRequest *)getTopicAttributesRequest {
    ...
    [request setEndpoint:[getTopicAttributesRequest requestEndpoint]];
    [request setRequestTag:[getTopicAttributesRequest requestTag]];

    if (getTopicAttributesRequest != nil) {
        if (getTopicAttributesRequest.topicArn != nil) {
            [request setParameterValue:[NSString stringWithFormat:@"%@", getTopicAttributesRequest.topicArn] forKey:[NSString stringWithFormat:@"%@", @"TopicArn"]];
        }
    }
