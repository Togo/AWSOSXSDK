...
- (void)parser:(NSXMLParser *)parser
    ...
    if ([elementName isEqualToString:@"VersionId"]) {
        self.summary.versionId = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"DisplayName"]) {
        self.summary.displayName = self.currentText;
        return;
    }

    if ([elementName isEqualToString:@"IsLatest"]) {
        self.summary.isLatest = [self.currentText boolValue];
        return;
    }
